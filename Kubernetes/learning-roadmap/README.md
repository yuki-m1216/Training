...
- **環境構築方針**: Dev/Prod環境としてファイルは極力分けず、ローカル環境（Kind）を使いながら本番環境を意識した構成で学習
  - Kustomizeの`overlays`は学習のために作成するが、実際にはdev環境のみを使用
  - リソース制限やレプリカ数などは学習に無理のない範囲で本番相当の設定を意識
  - 環境分離の複雑さよりも、Kubernetesの各機能の理解を優先
