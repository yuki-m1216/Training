...
8. **MMDSv2（2026-08-30 追加）**：2026-06-30 以降 Runtime は `metadataConfiguration.requireMMDSV2=true` が必須（未設定は `ValidationException`）。CFN スキーマに `MetadataConfiguration` が無く CDK で設定不可 → V2a で「最初の invoke が失敗する」を予想し、`update-agent-runtime --metadata-configuration requireMMDSV2=true` で回復を実測（`整理_システム全体像` §5-(b)）
