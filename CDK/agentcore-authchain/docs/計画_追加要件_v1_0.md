...
> **目的の補足（2026-08-30 ユーザー訂正）**：本計画の追加要件 3 点は「盛り込んだ仕様（要件固有）」である。プロジェクトにはもう 1 つの目的として **AgentCore を使った AI エージェントシステム全体を作り理解すること**があり、**要件固有でない部分（Runtime のデプロイ方式・ネットワーク・IAM・Observability・Gateway 認証・Policy 運用・エージェント実装）は公式ベストプラクティスに沿った形を目指す**。検証の都合で緩めた箇所は §3-5／仮決め事項表 §0 で「検証用の簡略化」と明記し、本番案を併記する。全体像は `整理_システム全体像_v1_0.md`（起票予定）に集約する。
...
2. Runtime の `customClaims` 不一致／クレーム欠落時の HTTP ステータス（401 か 403 か）と本文 → **確定（V2b、2026-09-06）：401、本文 `{"jsonrpc":"2.0","error":{"code":-32001,"message":"Authorization denied"}}`**（署名／`client_id`／期限切れも 401 で文言だけ異なる）
3. AgentCore Policy での**配列クレーム**（`agents`）の tag 表現（`like` で一致するか／Set か／JSON 文字列か）
4. フェデレーションユーザーの refresh_token 再発行時の `triggerSource` 値と `agents` 再計算 → **確定（V2b）：`TokenGeneration_RefreshTokens`、`agents` は再計算される**（マスタ行削除後の refresh で非注入）
...
8. **MMDSv2（2026-08-30 追加）**：2026-06-30 以降 Runtime は `metadataConfiguration.requireMMDSV2=true` が必須（未設定は `ValidationException`）。CFN スキーマに `MetadataConfiguration` が無く CDK で設定不可 → **確定（V2a）：CFN 作成の Runtime は既定で `requireMMDSV2: true`。予想の失敗は起きず、`update-agent-runtime` 不要**（`整理_システム全体像` §5-(b)）
...
- v1.0 追記（2026-08-30）目的の補足（要件固有 ＋ AgentCore 全体の理解・ベストプラクティス準拠）を冒頭に追加
- v1.0 追記（2026-08-30）§6 に MMDSv2 必須（CFN 未対応）を追加
