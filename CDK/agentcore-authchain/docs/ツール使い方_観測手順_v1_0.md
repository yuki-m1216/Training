...
- **`invoke_runtime.py`**：`AuthChainRuntimeStack` を Bearer で直接呼ぶ（JWT 設定の Runtime は SDK の `invoke_agent_runtime` では呼べないため HTTPS）。
  ...
  python3 scripts/invoke_runtime.py --token-file out/tokens_userA-oidc.json --tamper        # 署名バイトを 1 ビット反転（末尾 1 文字の書き換えは改ざんにならない。下記）
  ...
  ```
  4 ステップ（設定 → 送るトークンの予習 → HTTPS 組み立て（URL エンコードした ARN、`X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` は 33 文字以上）→ 応答（ステータス／`WWW-Authenticate`／本文））。`--save` で応答を `out/` に保存。
  - **実測（V2a/V2b、2026-09-06）**：通れば 200 と `received_header_names`／`authorization.claims`。拒否は**すべて 401**（`x-amzn-ErrorType: UnrecognizedClientException`、本文 JSON-RPC `-32001`、`WWW-Authenticate: Bearer resource_metadata=…`）。文言は `検証ログ_V2` の壊す実験表
  - **改ざん実験の落とし穴**：base64url の末尾 1 文字を変えても、RS256 署名（256 バイト＝342 文字）の末尾文字は上位 2 ビットしか使われず、デコード後のバイト列が同一になり得る（`A`→`B` は 200 で通った）。`tamper()` は署名バイトを反転してから再エンコードし、送る前にバイト列の差を確かめる
  - **再ログインせずにトークンを更新する（refresh_token グラント）**：マスタ変更の反映を見るときはブラウザ不要。Pre Token Gen は `TokenGeneration_RefreshTokens` で再実行され、`agents` が再計算される。応答に `refresh_token` は含まれない（元のものを使い続ける）
    ```bash
    python3 - <<'EOF'
    import json, urllib.request, urllib.parse
    o = json.load(open('out/identity-outputs.json'))['AuthChainIdentityStack']; t = json.load(open('out/tokens_userA-oidc.json'))
    body = urllib.parse.urlencode({'grant_type': 'refresh_token', 'client_id': o['ClientId'], 'refresh_token': t['refresh_token']}).encode()
    req = urllib.request.Request(o['TokenEndpoint'], data=body, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    nt = json.loads(urllib.request.urlopen(req).read()); nt['refresh_token'] = t['refresh_token']
    json.dump(nt, open('out/tokens_userA-oidc_refreshed.json', 'w'), indent=1)
    EOF
    chmod 600 out/tokens_userA-oidc_refreshed.json
    ```
  - **Runtime 側の確認**：`aws bedrock-agentcore-control get-agent-runtime --agent-runtime-id <id>`（`agentRuntimeVersion`／`requestHeaderConfiguration`／`metadataConfiguration`）、ログ群 `/aws/bedrock-agentcore/runtimes/<id>-DEFAULT`（ストリーム `runtime-logs-<sessionId>`）。CloudTrail には `InvokeAgentRuntime` は出ない（管理イベントのみの証跡）
...
- v1.0 追記（2026-09-06）§4 `invoke_runtime.py` の実測、改ざん実験の落とし穴、refresh_token グラント、Runtime 側の確認コマンド
