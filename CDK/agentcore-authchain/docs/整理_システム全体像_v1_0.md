...
flowchart TB
    ...
    subgraph RTS["AuthChainRuntimeStack（デプロイ済、V2b 状態）"]
        R1["Runtime authchain_inspect_headers（version 2）<br/>直接コードデプロイ（zip, arm64, Python 3.13）<br/>JWT: allowedClients＋customClaims agents CONTAINS inspect-headers<br/>allowlist Authorization（V2b）／MMDSv2 既定で有効"]
    ...
    U -->|"4 POST /invocations（Bearer）invoke_runtime.py → 200／401"| R1
...
flowchart LR
    ...
    A -. "未ヒット→クレーム非注入" .-> B
    B -. "欠落→401 Authorization denied（実測）" .-> C
```
| 層 | 拒否の条件 | 拒否の見え方 | 状態 |
|---|---|---|---|
| ① Cognito | IdP 認証失敗／クライアントに許可されていない IdP | hosted UI のエラー、`/callback?error=` | 実測済（V1） |
| ② Runtime | 署名・iss・`client_id`・`customClaims` のいずれか不一致、期限切れ | **すべて HTTP 401**（`WWW-Authenticate: Bearer resource_metadata=…`、本文 JSON-RPC `-32001`）。文言：署名 "Invalid Signature for bearerToken"／`client_id` "Claim 'client_id' value mismatch"／期限 "Token has expired"／customClaims "Authorization denied"（クレーム名を伏せる）。403 は出ない | 実測済（V2b） |
...
|---|---|---|---|---|
| **MMDSv2（必須）** | 2026-06-30 以降、`metadataConfiguration.requireMMDSV2=true` でない Runtime は**呼び出しが `ValidationException`** | CFN スキーマに `MetadataConfiguration` が**無い**（2026-09-06 再確認）→ CDK では設定不可 | **V2a 実測：CFN 作成の Runtime は既定で `requireMMDSV2: true`**（`get-agent-runtime`）。予想の ValidationException は起きず、`update-agent-runtime` 不要。ただし CDK で明示できない＝既定依存。CFN 対応後に CDK で明示する | ［A-2］［A-7］ |
...
| ヘッダ転送（`Authorization` allowlist） | 公式手順（Step 7）どおり allowlist に入れ、エージェントは署名検証せず decode | V2b で `allowlistedHeaders: ['Authorization']`、`PyJWT verify_signature=False` | **一致（V2b 実測）**。allowlist なしでは不達、ありで到達。`baggage`／`workloadaccesstoken` は常に届く | ［A-3］ |
...
| Observability | Transaction Search 有効化、Runtime はログ群自動、Gateway は vended logs 設定、ADOT で計装 | L2 `tracingEnabled`／`loggingConfigs` は未使用。ADOT 未導入 | **V2a 実測**：ログ群 `…/<id>-DEFAULT` は deploy 時に自動生成（保持期間 無期限）、`spans`／`otel-rt-logs` は空。CloudTrail は管理イベントのみで **`InvokeAgentRuntime` は記録されない**（データイベント未設定。本番案：データイベント有効化＋ログ保持期間の設定）。Transaction Search は V3 で判断 | ［A-5］ |
...
- v1.0 追記（2026-09-06）V2 完了を反映：§1-2 Runtime スタックを実線に、§4 ②の拒否を実測（すべて 401）、§5-(b) MMDSv2（既定 true）／ヘッダ転送／Observability の現状列
